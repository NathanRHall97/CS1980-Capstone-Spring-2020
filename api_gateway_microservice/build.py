...
# Use this funciton to dynamically assign test IPs in the test file
# For future tests you need to assign the microservice ip and write it in.
...
def make_dockercompose_file(microservices_dict, compose_dict):
    ...
    for name, ip_address in microservices_dict.items():
        ...
        new_microservice = {name: {"build": {'context': path, 'dockerfile': 'Dockerfile'},
    ...
    database_service = {"db_server": {'image': "postgres", 'container_name': "my_postgres",
                                      "networks": {"my_network": {"ipv4_address": db_ip}}, "ports": [("54320:5432")],
...
def main():
    ...
    api_paths = get_api_yaml_paths(
        MICROSERVICES_DIR)  # search files in the current directory and all sub-directories for api_doc.yaml paths
